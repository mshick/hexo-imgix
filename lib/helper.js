...
const buildSrc = function (path, params, settings) {
...
const buildSrcset = function (path, profile, settings) {
  ...
  const scaleSources = function (scl) {
    if (params.w && isScale(scl)) {
      ...
      const p = Object.assign({ fit: "clip" }, params);
      const width = Math.floor(parseInt(params.w, 10) * scl);
      p.w = scl.toFixed(2);
      delete p.h;
      if (width > min && width < max) {
        return {
          src: buildSrc(path, p, settings),
          width
        };
      }
    }
...
const buildAttrs = function (imageUrl, profile, settings) {
  ...
  attrs.src = buildSrc(imageUrl, profile.params, settings);
